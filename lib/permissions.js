 ...
 * @param {number} userPermissions - User's permission value
 * @returns {string} - Permission type: 'admin', 'request-manager', 'manager', 'auto-approve', or 'user'
 */
export const getPermissionType = (userPermissions) => {
  ...
  // Request manager check - can approve requests (MANAGE_REQUESTS only)
  if (hasPermission(Permission.MANAGE_REQUESTS, userPermissions)) {
    return 'request-manager';
  }

  // General manager check - has other management permissions but NOT request approval
  const otherManagementPermissions = [
  ...
  if (hasPermission(otherManagementPermissions, userPermissions, { type: 'or' })) {
    return 'manager';
  }
 ...
 * CRITICAL: Only ADMIN (2) or MANAGE_REQUESTS (16) permissions grant approval rights
 * MANAGE_ISSUES (1048576) and other permissions do NOT grant approval rights
 * @param {number} userPermissions - User's permission value
 * @returns {boolean} - True if user can approve requests
 */
export const canApproveRequests = (userPermissions) => {
  // Explicit check: only ADMIN (2) or MANAGE_REQUESTS (16) can approve
  return hasPermission(Permission.ADMIN, userPermissions) || 
         hasPermission(Permission.MANAGE_REQUESTS, userPermissions);
};
