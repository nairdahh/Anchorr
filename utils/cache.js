...
class APICache {
  ...
  getStats() {
    ...
    const jellyseerrTotal = this.stats.jellyseerrHits + this.stats.jellyseerrMisses;
    ...
    return {
      tmdb: {
        ...
        hitRate: tmdbTotal > 0 ? ((this.stats.tmdbHits / tmdbTotal) * 100).toFixed(2) + '%' : '0%'
      },
      jellyseerr: {
        keys: jellyseerrKeys,
        hits: this.stats.jellyseerrHits,
        misses: this.stats.jellyseerrMisses,
        hitRate: jellyseerrTotal > 0 ? ((this.stats.jellyseerrHits / jellyseerrTotal) * 100).toFixed(2) + '%' : '0%'
      }
    };
  }
}
